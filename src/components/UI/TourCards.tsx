...
const TourCards = ({ locale }: TourCardsProps) => {
  ...
  const tours: TourCard[] = [
    ...
    { nameKey: "Tours.classicTours.central.cardLabel", href: "/tours/central", imageSrc: "/tours/Central1.jpg", popular: true },
    { nameKey: "Tours.classicTours.zavkhan.cardLabel", href: "/tours/zavkhan", imageSrc: "/tours/tour-zavkhan/card.jpg", popular: true },
    { nameKey: "KhuvsgulSelection.cardLabel", href: "/tours/khuvsgul", imageSrc: "/tours/tour-khuvsgul1/huvsgul.jpg", popular: true },
  ];
      ...
      {tours.map((tour, index) => (
        <div key={index} className={styles.tour_card_wrapper}>
          {SHOW_POPULAR_BADGE && tour.popular && (
            <div className={`${styles.tour_card_popular_badge} ${validLocale === "en" ? styles.tour_card_popular_badge_en : ""}`} aria-hidden>
              <div>{t("Tours.popularProduct")}</div>
              <div>{t("Tours.product")}</div>
            </div>
          )}
          <Link
